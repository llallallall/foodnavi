  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-12 bg-background p-8 md:p-14 rounded-none shadow-xl border border-border/50 relative overflow-hidden">
      ...
      <div className="text-center mb-10">
        <h2 className="text-2xl font-serif font-bold text-foreground mb-2">프리미엄 무료 진단 신청서</h2>
        <p className="text-foreground/70 text-sm">정확한 처방을 위해 현재 상황을 구체적으로 알려주세요.</p>
      </div>
      ...
      <div className="bg-background dark:bg-secondary border border-border p-8 md:p-10 rounded-none relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">회사명</Label>
            <Input {...register("company_name")} error={!!errors.company_name} placeholder="예: 윤앤파트너스 주식회사" className="h-12 bg-background" />
            {errors.company_name && <p className="text-xs text-destructive">{errors.company_name.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">담당자명 (직위, 이름)</Label>
            <Input {...register("contact_name")} error={!!errors.contact_name} placeholder="예: 홍길동 과장" className="h-12 bg-background" />
            {errors.contact_name && <p className="text-xs text-destructive">{errors.contact_name.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">이메일 주소</Label>
            <Input type="email" {...register("contact_email")} error={!!errors.contact_email} placeholder="결과 리포트를 받으실 이메일" className="h-12 bg-background" />
            {errors.contact_email && <p className="text-xs text-destructive">{errors.contact_email.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">휴대전화 번호</Label>
            <Input {...register("contact_phone_mobile")} error={!!errors.contact_phone_mobile} placeholder="010-0000-0000" className="h-12 bg-background" />
      ...
      <div className="bg-background dark:bg-secondary border border-border p-8 md:p-10 rounded-none relative">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">수출용 제품명</Label>
              <Input {...register("product_name")} error={!!errors.product_name} placeholder="한글/영문 제품명 기재" className="h-12 bg-background" />
              {errors.product_name && <p className="text-xs text-destructive">{errors.product_name.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">제품군 (품목군)</Label>
              <Input {...register("category")} error={!!errors.category} placeholder="가공식품, 수산물 등 (HS코드를 아신다면 함께 기재)" className="h-12 bg-background" />
              {errors.category && <p className="text-xs text-destructive">{errors.category.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">내용량</Label>
              <Input {...register("net_weight")} error={!!errors.net_weight} placeholder="예: 500g" className="h-12 bg-background" />
              {errors.net_weight && <p className="text-xs text-destructive">{errors.net_weight.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">원산국</Label>
              <Input {...register("country_of_origin")} error={!!errors.country_of_origin} placeholder="예: 국산, 중국산 등" className="h-12 bg-background" />
              {errors.country_of_origin && <p className="text-xs text-destructive">{errors.country_of_origin.message}</p>}
            </div>
          </div>

          <div className="space-y-2">
            <Label required className="text-foreground font-bold">원재료 및 성분명 (첨가물 포함)</Label>
            <p className="text-xs text-foreground/50 mb-2">원재료별 비중(%)을 반드시 입력해 주십시오. 통관 가능 여부를 결정하는 가장 중요한 항목입니다.</p>
            <Textarea {...register("product_description")} error={!!errors.product_description} placeholder="예: 배추(국산) 70%, 고춧가루(국산) 15%, 마늘 5%, L-글루탐산나트륨 0.5% 등" className="min-h-[140px] bg-background text-base p-4" />
            {errors.product_description && <p className="text-xs text-destructive">{errors.product_description.message}</p>}
          </div>

          <div className="space-y-2">
            <Label required className="text-foreground font-bold">영양성분표</Label>
            <Textarea {...register("nutrition_facts")} error={!!errors.nutrition_facts} placeholder="칼로리, 탄수화물, 단백질, 지방, 나트륨 등 영양성분 정보 기재" className="min-h-[100px] bg-background text-base p-4" />
            {errors.nutrition_facts && <p className="text-xs text-destructive">{errors.nutrition_facts.message}</p>}
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">유통기한 (제조일로부터)</Label>
              <Input {...register("shelf_life")} error={!!errors.shelf_life} placeholder="예: 제조일로부터 12개월" className="h-12 bg-background" />
              {errors.shelf_life && <p className="text-xs text-destructive">{errors.shelf_life.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">보존방법 (보관온도)</Label>
              <Input {...register("storage_method")} error={!!errors.storage_method} placeholder="예: 실온 보관, 냉동 보관(-18℃ 이하)" className="h-12 bg-background" />
              {errors.storage_method && <p className="text-xs text-destructive">{errors.storage_method.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">섭취방법</Label>
              <Input {...register("consumption_method")} error={!!errors.consumption_method} placeholder="예: 그대로 섭취, 데워서 섭취" className="h-12 bg-background" />
              {errors.consumption_method && <p className="text-xs text-destructive">{errors.consumption_method.message}</p>}
            </div>
            <div className="space-y-2">
              <Label required className="text-foreground font-bold">가열조리 필요 여부</Label>
              <Input {...register("cooking_instructions")} error={!!errors.cooking_instructions} placeholder="필요 시 가열 방법 및 시간 기재" className="h-12 bg-background" />
              {errors.cooking_instructions && <p className="text-xs text-destructive">{errors.cooking_instructions.message}</p>}
            </div>
          </div>

          <div className="space-y-2">
            <Label className="text-foreground font-bold">냉동식품의 경우 (제조 시 가열 온도 및 시간)</Label>
            <Input {...register("frozen_heating_info")} error={!!errors.frozen_heating_info} placeholder="해당되는 경우에만 기재 (예: 95℃에서 10분 가열)" className="h-12 bg-background" />
          </div>

          <div className="space-y-2">
            <Label required className="text-foreground font-bold">제조공정도</Label>
            <Textarea {...register("manufacturing_flowchart")} error={!!errors.manufacturing_flowchart} placeholder="원료 입고부터 포장까지의 생산 공정 순서를 기재해주세요." className="min-h-[100px] bg-background text-base p-4" />
            {errors.manufacturing_flowchart && <p className="text-xs text-destructive">{errors.manufacturing_flowchart.message}</p>}
          </div>

          <div className="space-y-2">
            <Label required className="text-foreground font-bold">제품의 특장점 (셀링 포인트)</Label>
            <Textarea {...register("features")} error={!!errors.features} placeholder="타사 제품 대비 경쟁력, 주 타겟층 등을 기재해주세요." className="min-h-[100px] bg-background text-base p-4" />
      ...
      <div className="bg-background dark:bg-secondary border border-border p-8 md:p-10 rounded-none relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2 md:col-span-2">
            ...
            <Input {...register("sales_channel")} error={!!errors.sales_channel} placeholder="백화점, 이마트, 자사몰 등" className="h-12 bg-background" />
            {errors.sales_channel && <p className="text-xs text-destructive">{errors.sales_channel.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">납품가격 (FOB 기준)</Label>
            <Input {...register("fob_price")} error={!!errors.fob_price} placeholder="최소주문수량(MOQ) 기준" className="h-12 bg-background" />
            {errors.fob_price && <p className="text-xs text-destructive">{errors.fob_price.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">공장 출고 가격</Label>
            <Input {...register("exw_price")} error={!!errors.exw_price} placeholder="최소주문수량(MOQ) 기준" className="h-12 bg-background" />
            {errors.exw_price && <p className="text-xs text-destructive">{errors.exw_price.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">발주 리드타임</Label>
            <Input {...register("lead_time")} error={!!errors.lead_time} placeholder="발주 후 제조하여 일본 항구 도착까지 기간" className="h-12 bg-background" />
            {errors.lead_time && <p className="text-xs text-destructive">{errors.lead_time.message}</p>}
          </div>
          <div className="space-y-2">
            <Label required className="text-foreground font-bold">보유 인허가 및 인증</Label>
            <Input {...register("certifications")} error={!!errors.certifications} placeholder="HACCP, ISO22000, 비건 인증 등" className="h-12 bg-background" />
            {errors.certifications && <p className="text-xs text-destructive">{errors.certifications.message}</p>}
          </div>
          <div className="space-y-2 md:col-span-2">
            <Label required className="text-foreground font-bold">해외 수출 경험 여부</Label>
            <Input {...register("export_experience")} error={!!errors.export_experience} placeholder="없음 / 대만 수출 1년 등" className="h-12 bg-background" />
      ...
      <div className="bg-background dark:bg-secondary border border-border p-8 md:p-10 rounded-none relative">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <Label required className="text-foreground font-bold">일본어 카탈로그 보유 여부</Label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
          ...
          <div className="space-y-2">
            <Label className="text-foreground font-bold block">관련 파일 업로드 
              <span className="text-xs text-foreground/50 font-normal ml-2 tracking-normal">(카탈로그, 라벨, 제품 이미지 등 / 10MB 이하)</span>
            </Label>
            <div className="border-2 border-dashed border-border p-6 text-center bg-background hover:bg-muted/30 transition-colors cursor-pointer rounded-none">
              ...
              <p className="font-medium text-sm text-foreground">파일을 드래그하거나 클릭하여 업로드</p>
              <p className="text-xs text-foreground/50 mt-1 text-center max-w-sm mx-auto">
          ...
          <div className="space-y-2 pt-4 border-t border-border">
            <Label className="text-foreground font-bold">기타 문의사항</Label>
            <Textarea {...register("other_inquiries")} placeholder="궁금하신 점이나 추가로 전달하실 내용을 자유롭게 적어주세요." className="min-h-[100px] bg-background text-base p-4" />
          </div>
        </div>
      </div>
